...
export function registerSchemaUiTools(server, db) {
    server.registerTool("list_schemas", {
        title: "List Schemas",
        description: "List all active schemas from MongoDB. Use this to discover available schema keys before fetching schema and React base code.",
        inputSchema: {},
    }, async () => {
    ...
    server.registerTool("get_schema_with_react_base", {
        title: "Get Schema with React Base Code",
        description: "Fetches a schema from MongoDB and returns the schema JSON plus the full React base code (SchemaForm.tsx, types, API client). This is the base UI for rendering the schema. The user can customize colors, typography, positions. Use this when the user wants to modify the form UI.",
        inputSchema: {
    ...
    server.registerTool("get_schema", {
        title: "Get Schema",
        description: "Retrieve a schema by its key from MongoDB. Returns the schema JSON with field definitions. Use this when you only need the schema (e.g. to render a form or understand field structure) without the React base code.",
        inputSchema: {
            schemaKey: z.string().describe("Schema key (e.g. hold_entry:add:v1)"),
        },
    }, async ({ schemaKey }) => {
