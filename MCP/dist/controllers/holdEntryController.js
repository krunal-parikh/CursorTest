...
export async function previewHoldEntry(req, res, next) {
    try {
        const db = getDb();
        const holdService = createHoldEntryService(db);
        const { schemaKey, data } = req.body;
...
export async function createHoldEntry(req, res, next) {
    try {
        const db = getDb();
        const holdService = createHoldEntryService(db);
        const { schemaKey, data } = req.body;
        const payload = data ?? req.body;
        const schemaKeyToUse = schemaKey ?? "hold_entry:add:v1";
        const entry = await holdService.createHoldEntry(schemaKeyToUse, payload, req.requestId);
...
export async function getHoldEntry(req, res, next) {
    try {
...
export async function listHoldEntries(req, res, next) {
    try {
        const db = getDb();
        const holdService = createHoldEntryService(db);
...
//# sourceMappingURL=holdEntryController.js.map