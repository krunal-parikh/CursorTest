...
import { assertValidHoldId, normalizeListParams, validateHoldEntryData, } from "../domain/validateHoldEntryData.js";
const ALLOWED_COLLECTIONS = ["hold_entries"];
export function createHoldEntryService(db) {
    ...
    return {
        async previewHoldEntry(schemaKey, data) {
        ...
        async createHoldEntry(schemaKey, data, requestId) {
            const schema = await schemaRepo.findByKey(schemaKey);
            if (!schema)
                throw new Error(`Schema not found: ${schemaKey}`);
            const enriched = await applyAutoPopulate(schema, data);
        ...
        async createRecord(collection, payload, requestId) {
            if (!ALLOWED_COLLECTIONS.includes(collection)) {
                throw new AppValidationError(`Collection not allowed. Allowed: ${ALLOWED_COLLECTIONS.join(", ")}`, [{ path: "collection", message: `Must be one of: ${ALLOWED_COLLECTIONS.join(", ")}` }]);
            }
            if (collection === "hold_entries") {
                const schemaKey = String(payload.schemaKey ?? "hold_entry:add:v1");
                const { data: payloadData, ...rest } = payload;
                const data = payloadData ?? rest;
        ...
        async getHoldEntry(holdId) {
            assertValidHoldId(holdId);
            return holdRepo.findByHoldId(holdId.trim());
        },
        async listHoldEntries(filter, limit, page) {
...
async function applyAutoPopulate(schema, data) {
...
//# sourceMappingURL=holdEntryService.js.map