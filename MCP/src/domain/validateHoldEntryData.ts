...
import type { FieldConfig, SchemaDocument } from "../types/schema.js";

const DATE_RE = /^(0[1-9]|1[0-2])\/(0[1-9]|[12]\d|3[01])\/\d{4}$/;

function issuesError(issues: Array<{ path: string; message: string }>): AppValidationError {
...
function isEmptyValue(field: FieldConfig, raw: unknown): boolean {
...
function applyFieldRules(
  ...
  if (isEmptyValue(field, raw)) return;
 ...
 */
export function validateHoldEntryData(schema: SchemaDocument, data: Record<string, unknown>): void {
...
export function assertValidHoldId(holdId: string): void {
  const t = holdId?.trim() ?? "";
  if (!t) {
