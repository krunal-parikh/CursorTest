...
import type { SelectOption } from "../types/schema.js";
import { withValidationToolResult } from "./mcpToolResult.js";

export function registerThresholdTools(
  server: McpServer,
  deps: {
  ...
  server.registerTool(
    "get_hold_entry_search_fields",
    {
      title: "Get Hold Entry Search Fields",
      description:
        "Returns all searchable fields with format and allowed values. Call this BEFORE list_hold_entries to know valid filter values. Use plain labels for the user.",
      inputSchema: {},
    },
    async () => {
      ...
      if (!schema) {
        return {
          content: [{ type: "text" as const, text: "Schema hold_entry:add:v1 not found." }],
          isError: true,
        };
      }
      ...
      const text = [
        ...
        ...searchFields.map(
          (f) =>
            `- ${f.label} (${f.field_name}): ${f.searchFormat} match. Allowed: ${JSON.stringify(f.limitation)}`
        ),
      ].join("\n");

      return {
        content: [{ type: "text" as const, text }],
        structuredContent: { searchFields } as Record<string, unknown>,
      };
    }
  );
  ...
  server.registerTool(
    "get_hold_entry_options",
    {
      title: "Get Hold Entry Options",
      description:
        "Returns all dropdown choices for creating a hold. ALWAYS call this and SHOW the options to the user before asking them to pick. Use plain language (e.g. 'Category: Product Safety RedFlag, Product Quality RedFlag...'). Never mention schema keys or technical IDs.",
      inputSchema: {},
    },
    async () => {
      const schema = await schemaService.getSchema("hold_entry:add:v1");
      ...
      const formatOptions = (opts: SelectOption[]) =>
        opts.map((o, idx) => `${idx + 1}. ${o.label ?? o.value}`).join(", ");
  ...
  server.registerTool(
    "get_hold_entry_workflow",
    {
      title: "Get Hold Entry Workflow",
      description:
        "Returns the step-by-step flow for creating a hold. Call get_hold_entry_options FIRST and SHOW options to the user. Use plain language only - no schema keys or IDs.",
      inputSchema: {},
    },
    async () => {
      const workflow = {
        ...
        instructions:
          "ALWAYS show options before asking. Use plain labels like 'Category', 'MO # / BATCH #', 'Production Date'. Never mention schema keys, field names, or technical IDs.",
      };
      return {
        content: [
  ...
  server.registerTool(
    "preview_hold_entry",
    {
      ...
      description:
        "Shows what will be created WITHOUT submitting. Use this to show a summary to the user before they confirm. Call create_record only after user confirms. Dates must be mm/dd/yyyy - convert natural language on your side.",
      inputSchema: {
    ...
    async (params) => {
      return withValidationToolResult(async () => {
        ...
        const { enriched, holdIdPreview } = await holdService.previewHoldEntry(
          "hold_entry:add:v1",
          data
        );
  ...
  server.registerTool(
    "create_record",
    {
      title: "Create Record",
      description:
        "SUBMIT a hold entry. Call ONLY after: (1) user has seen the preview from preview_hold_entry, and (2) user has confirmed (e.g. 'yes submit', 'confirm'). Never call without confirmation.",
      inputSchema: {
        collection: z
          .enum(["hold_entries"])
    ...
    async ({ collection, data }) => {
      return withValidationToolResult(async () => {
        const result = await holdService.createRecord(
          collection,
          { schemaKey: "hold_entry:add:v1", ...data },
          undefined
        );
        return {
          content: [
  ...
  server.registerTool(
    "list_hold_entries",
    {
      ...
      description:
        "Search and list existing hold entries. Call get_hold_entry_search_fields FIRST to see valid filter values. You can filter by ANY field: Hold ID, Status, Plant, MO/Batch, Category, Sub-Category, Observation, Location, etc.",
      inputSchema: {
        filter: z
          .record(z.unknown())
          .optional()
          .describe(
            "Filter object. Keys: holdId, status, plantCode, moBatchId, prodDate, category, subCategory, observation, qtyOnHoldCs, plantRelated, location, lpsHus, status, dispositionProvider, disposition, dispositionActor. Use get_hold_entry_search_fields for valid values."
          ),
    ...
    async ({ filter, limit, page }) => {
      return withValidationToolResult(async () => {
        const result = await holdService.listHoldEntries(
          filter ?? {},
          limit ?? 20,
          page ?? 1
        );
  ...
  server.registerTool(
    "get_hold_entry",
    {
      ...
      description:
        "Get a single hold entry by its Hold ID (e.g. PLT01-000001). Use when the user asks for details of a specific hold.",
      inputSchema: {
    ...
    async ({ holdId }) => {
      return withValidationToolResult(async () => {
        ...
        if (!entry) {
          return {
            content: [{ type: "text" as const, text: `Hold entry not found: ${holdId}` }],
            isError: true,
          };
        }
        const record = {
